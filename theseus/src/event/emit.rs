...
/// Общая функция для отправки событий во frontend
#[cfg(feature = "tauri")]
pub async fn emit_event(
    ...
    let event_state = EventState::get().await?;
    // В Tauri 1.x используем emit_event вместо app.emit
    event_state.app.emit_all(event, payload)?;
    Ok(())
}

/// Общая функция для отправки событий (без Tauri)
#[cfg(not(feature = "tauri"))]
pub async fn emit_event(
    _event: &str,
    _payload: serde_json::Value,
) -> crate::Result<()> {
    // В режиме без Tauri просто игнорируем события
    Ok(())
}
