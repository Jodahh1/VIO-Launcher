...
use crate::state::{AccountType, Credentials, ProfileInstallStage};
use crate::util::io;
use crate::{
    process,
...
pub use authlib_injector::{
    check_ely_by_availability,
    get_or_download_authlib,
};
...
pub async fn launch_minecraft(
    ...
    if matches!(credentials.account_type, AccountType::ElyBy { .. }) {
        get_or_download_authlib(None).await?;
        command.arg(authlib_injector::get_authlib_agent_arg().await?);
    }
