...
impl MinecraftAuthStore {
    ...
    pub async fn get_default_credential(
        ...
        if let Some(creds) = credentials {
            ...
            if creds.expires < Utc::now() {
                let val = match &creds.account_type {
                    ...
                    AccountType::Offline => creds.clone(),
                    AccountType::ElyBy { profile_id } => {
                        let client_token = creds.client_token.clone().ok_or_else(|| {
                            crate::ErrorKind::OtherError(
                                "Ely.by client token is missing".to_string(),
                            )
                            .as_error()
                        })?;

                        let refresh = ely_auth::refresh(&creds.access_token, &client_token).await?;
                        Credentials {
                            id: parse_uuid_or_random(profile_id),
                            username: refresh.selected_profile.name,
                            ...
                            account_type: AccountType::ElyBy {
                                profile_id: refresh.selected_profile.id.clone(),
                            },
                            client_token: Some(refresh.client_token),
                            ely_data: Some(ElyByData {
                                uuid: refresh.selected_profile.id,
                                email: refresh.user.and_then(|user| user.email),
                            }),
                        }
                    }
                };
    ...
    pub async fn ely_login_finish(
        &mut self,
        username: &str,
        password: &str,
    ) -> crate::Result<Credentials> {
        let client_token = Uuid::new_v4().to_string();
        let response = ely_auth::authenticate(username, password, &client_token).await?;
        let profile_id = response.selected_profile.id.clone();
        let id = parse_uuid_or_random(&profile_id);
        ...
        let credentials = Credentials {
            ...
            ely_data: Some(ElyByData {
                uuid: profile_id,
                email: response.user.and_then(|user| user.email),
            }),
        };

        self.users.insert(id, credentials.clone());
...
fn parse_uuid_or_random(value: &str) -> Uuid {
    ...
    if trimmed.len() == 32 {
        ...
        if let Ok(uuid) = Uuid::parse_str(&hyphenated) {
            return uuid;
        }
    }
