...
pub async fn authenticate_begin_flow(provider: &str) -> Result<String> {
    Ok(vio::mr_auth::authenticate_begin_flow(provider).await?)
}

#[tauri::command]
pub async fn authenticate_await_completion() -> Result<ModrinthCredentialsResult>
{
    Ok(vio::mr_auth::authenticate_await_complete_flow().await?)
}

#[tauri::command]
pub async fn cancel_flow() -> Result<()> {
    Ok(vio::mr_auth::cancel_flow().await?)
}
...
pub async fn login_pass(
    ...
    Ok(vio::mr_auth::login_password(username, password, challenge).await?)
}

#[tauri::command]
pub async fn login_2fa(code: &str, flow: &str) -> Result<ModrinthCredentials> {
    Ok(vio::mr_auth::login_2fa(code, flow).await?)
}
...
pub async fn refresh() -> Result<()> {
    Ok(vio::mr_auth::refresh().await?)
}

#[tauri::command]
pub async fn logout() -> Result<()> {
    Ok(vio::mr_auth::logout().await?)
}

#[tauri::command]
pub async fn get() -> Result<Option<ModrinthCredentials>> {
    Ok(vio::mr_auth::get_credentials().await?)
}
