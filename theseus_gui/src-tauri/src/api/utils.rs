...
) -> Result<std::collections::HashMap<uuid::Uuid, vio::LoadingBar>> {
    let res = vio::EventState::list_progress_bars().await?;
    Ok(res)
}
...
pub async fn safety_check_safe_loading_bars() -> Result<bool> {
    Ok(vio::safety::check_safe_loading_bars().await?)
}
...
pub async fn handle_command(command: String) -> Result<()> {
    Ok(vio::handler::parse_and_emit_command(&command).await?)
}
