...
// Serializable error intermediary, so TheseusGuiError can be Serializable (eg: so that we can return vio::Errors in Tauri directly)
#[derive(Error, Debug)]
pub enum TheseusSerializableError {
    #[error("{0}")]
    Theseus(#[from] vio::Error),
