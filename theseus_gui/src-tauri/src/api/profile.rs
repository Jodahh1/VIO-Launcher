...
use vio::{prelude::*, InnerProjectPathUnix};
use uuid::Uuid;
...
pub fn init<R: tauri::Runtime>() -> tauri::plugin::TauriPlugin<R> {
    tauri::plugin::Builder::new("profile")
        .invoke_handler(tauri::generate_handler![
            ...
            profile_screenshots_list,
            profile_screenshot_delete,
            profile_screenshot_open,
            profile_optimize_memory,
            profile_optimize_jvm,
            profile_check_conflicts,
            profile_check_updates,
            profile_install_updates,
            profile_set_skin,
            profile_get_skins,
            profile_get_console_logs,
            profile_scan_errors,
        ])
        .build()
}
...
// Экспорт профиля в backup
#[tauri::command]
pub async fn profile_backup_export(
    path: ProfilePathId,
    output_path: PathBuf,
) -> Result<vio::api::profile::backup::BackupMetadata> {
    let metadata = profile::backup::export_profile(&path, &output_path).await?;
    Ok(metadata)
}

// Импорт профиля из backup
#[tauri::command]
pub async fn profile_backup_import(
    archive_path: PathBuf,
    target_name: Option<String>,
) -> Result<ProfilePathId> {
    let path = profile::backup::import_profile(&archive_path, target_name).await?;
    Ok(path)
}

// Получить список скриншотов
#[tauri::command]
pub async fn profile_screenshots_list(
    path: ProfilePathId,
) -> Result<Vec<vio::api::profile::screenshot::ScreenshotInfo>> {
    let screenshots = profile::screenshot::list_screenshots(&path).await?;
    Ok(screenshots)
}

// Удалить скриншот
#[tauri::command]
pub async fn profile_screenshot_delete(
    path: ProfilePathId,
    screenshot_name: String,
) -> Result<()> {
    profile::screenshot::delete_screenshot(&path, &screenshot_name).await?;
    Ok(())
}

// Открыть скриншот
#[tauri::command]
pub async fn profile_screenshot_open(
    path: ProfilePathId,
    screenshot_name: String,
) -> Result<()> {
    profile::screenshot::open_screenshot(&path, &screenshot_name).await?;
    Ok(())
}

// Оптимизировать память
#[tauri::command]
pub async fn profile_optimize_memory(
    path: ProfilePathId,
) -> Result<vio::api::profile::optimize::MemoryRecommendation> {
    let rec = profile::optimize::optimize_memory(&path).await?;
    Ok(rec)
}

// Оптимизировать JVM аргументы
#[tauri::command]
pub async fn profile_optimize_jvm(
    path: ProfilePathId,
) -> Result<Vec<String>> {
    let args = profile::optimize::optimize_jvm_args(&path).await?;
    Ok(args)
}

// Проверить конфликты модов
#[tauri::command]
pub async fn profile_check_conflicts(
    path: ProfilePathId,
) -> Result<vio::api::profile::conflict::ConflictReport> {
    let report = profile::conflict::check_conflicts(&path).await?;
    Ok(report)
}
...
pub async fn profile_check_updates(
    path: ProfilePathId,
) -> Result<vio::api::profile::auto_update::UpdateCheckResult> {
    let result = profile::auto_update::check_for_updates(&path).await?;
    Ok(result)
}

// Установить все обновления
#[tauri::command]
pub async fn profile_install_updates(
    path: ProfilePathId,
) -> Result<vio::api::profile::auto_update::UpdateCheckResult> {
    let result = profile::auto_update::install_all_updates(&path).await?;
    Ok(result)
}

// Установить кастомный скин
#[tauri::command]
pub async fn profile_set_skin(
    ...
    skin_path: PathBuf,
    skin_type: String,
) -> Result<vio::api::profile::skin::SkinInfo> {
    let skin_type = if skin_type == "slim" {
        vio::api::profile::skin::SkinType::Slim
    } else {
        vio::api::profile::skin::SkinType::Steve
    };
    let info = profile::skin::set_offline_skin(&path, &player_name, &skin_path, skin_type).await?;
    Ok(info)
}

// Получить все скины
#[tauri::command]
pub async fn profile_get_skins(
    path: ProfilePathId,
) -> Result<Vec<vio::api::profile::skin::SkinInfo>> {
    let skins = profile::skin::get_skins(&path).await?;
    Ok(skins)
}

// Получить логи консоли
#[tauri::command]
pub async fn profile_get_console_logs(
    path: ProfilePathId,
    limit: usize,
) -> Result<Vec<vio::api::profile::console_logger::LogMessage>> {
    let logs = profile::console_logger::get_recent_logs(&path, limit).await?;
    Ok(logs)
}

// Сканировать логи на ошибки
#[tauri::command]
pub async fn profile_scan_errors(
    path: ProfilePathId,
) -> Result<Vec<vio::api::profile::console_logger::LogMessage>> {
    let errors = profile::console_logger::scan_for_errors(&path).await?;
    Ok(errors)
}
