...
async fn toggle_decorations(b: bool, window: tauri::Window) -> api::Result<()> {
    window.set_decorations(b).map_err(|e| {
        vio::Error::from(vio::ErrorKind::OtherError(format!(
...
fn main() {
    tauri_plugin_deep_link::prepare("com.vio.launcher");
    ...
    */
    let _log_guard = vio::start_logger();

    tracing::info!("Initialized tracing subscriber. Loading VIO Launcher!");
    ...
    let builder = builder
        ...
        .plugin(api::account::init())
        .plugin(api::ely_auth_plugin::init())
