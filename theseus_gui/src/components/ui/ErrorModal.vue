...
const supportLink = ref(supportTelegram)
const metadata = ref({})
...
defineExpose({
  async show(errorVal) {
    if (errorVal.message && errorVal.message.includes('Minecraft authentication error:')) {
      ...
      supportLink.value = supportTelegram
      ...
      <div class="markdown-body">
        <template v-if="errorType === 'minecraft_auth'">
          ...
          <template v-if="metadata.network">
            ...
            <p>
              It looks like there were issues with VIO Launcher connecting to Microsoft's
              servers. This is often the result of a poor connection, so we recommend trying again
              to see if it works. If issues continue to persist, try the official Minecraft
              Launcher first and then contact support in Telegram.
            </p>
          </template>
          <template v-else-if="metadata.hostsFile">
            <h3>Network issues</h3>
            <p>
              VIO Launcher tried to connect to Microsoft / Xbox / Minecraft services, but the
              remote server rejected the connection. This may indicate that these services are
              blocked by the hosts file. If that keeps happening, contact support in Telegram and
              attach the error text below.
            </p>
          </template>
          <template v-else>
          ...
          <hr />
          <p>
            If nothing is working and you need help, open
            <a :href="supportLink">Telegram support</a>
            and send the error text below.
          </p>
          <details>
        ...
        <div v-else-if="errorType === 'minecraft_sign_in'">
          <div class="warning-banner">
            ...
            <div class="warning-banner__description">
              VIO Launcher now uses a newer sign-in flow for better stability and security. Please
              sign in again so your saved credentials can be refreshed correctly.
            </div>
          </div>
          <p>
